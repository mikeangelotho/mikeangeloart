...
export function CollectionCell({
    ...
    function SlideshowCover() {
        ...
        const [randIdx, setRandIdx] = createSignal(Math.floor(Math.random() * keypointMedia().length));
        ...
        const handlePrev = (e: MouseEvent) => {
            e.stopPropagation();
            setRandIdx((prev) => (prev - 1 + keypointMedia().length) % keypointMedia().length);
        };
    ...
    return (
        ...
        >
            ...
            <div class="min-h-18 lg:min-h-12 justify-center lg:rounded-3xl w-full lg:w-auto lg:inset-x-3 flex flex-col gap-2 def__animate group/tag group-hover/card:opacity-100 p-6 lg:opacity-0 absolute bottom-0 lg:bottom-3 z-20 bg-white/50 dark:bg-black/30 mx-auto backdrop-blur-xl backdrop-brightness-140 dark:backdrop-brightness-25">
