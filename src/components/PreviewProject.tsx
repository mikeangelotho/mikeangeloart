...
export function PreviewProject(props: {
    ...
    return (
        <section class="px-6 z-1 border-b-36 border-black/10 dark:border-white/5 w-full mx-auto py-36">
            <header class="w-full z-1 text-black dark:text-white max-w-7xl mx-auto">
                <div class="flex flex-col lg:flex-row pb-36 gap-12 justify-center items-center w-full">
                    <div class="flex flex-col items-center lg:flex-row gap-6">
                        ...
                        <article ref={objectiveContainer} class="fade__animate group max-w-3xl text-black dark:text-white shadow-[0px_9px_18px_0px_rgb(0,0,0,0.1)] dark:shadow-[0px_9px_18px_0px_rgb(0,0,0,0.25)] rounded-3xl p-6 items-center flex gap-6 flex-col-reverse lg:flex-row bg-neutral-100 dark:bg-black border border-black/10 dark:border-white/5 dark:border-t dark:border-t-white">
            ...
            <div
                class="z-1 w-full flex flex-col gap-36 justify-center items-center max-w-7xl mx-auto"
            >
                <MediaSpread data={props.data}>
                    <article class="text-black max-w-3xl dark:text-white shadow-[0px_9px_18px_0px_rgb(0,0,0,0.1)] dark:shadow-[0px_9px_18px_0px_rgb(0,0,0,0.25)] rounded-3xl p-6 items-center flex gap-6 flex-col-reverse lg:flex-row bg-neutral-100 dark:bg-black border border-black/10 dark:border-white/5 dark:border-t dark:border-t-white">
                        {/*
...
const MediaSpread = (props: { data: PortfolioCollection, children: JSX.Element }) => {
    ...
    let msGroups!: HTMLDivElement, msGroup1!: HTMLDivElement, msGroup2!: HTMLDivElement;
    ...
    return (
        <div ref={msGroups} class="fade__animate w-full flex gap-18 flex-col">
            <A href={`/projects/${props.data.slug}`}><MsMedia class="hover:-translate-y-3 def__animate aspect-video object-cover w-full max-w-5xl mx-auto rounded-3xl border-6 border-neutral-200 dark:border-white/5" src={urls[0]} /></A>
            <div ref={msGroup1} class="fade__animate lg:pr-96 flex flex-col lg:flex-row justify-center lg:items-end gap-18 w-full">
                 <A href={`/projects/${props.data.slug}`}><MsMedia class="hover:-translate-y-3 def__animate w-full aspect-square object-cover max-h-96 lg:max-h-54 rounded-3xl border-6 border-neutral-200 dark:border-white/5 -mb-12" src={urls[1]} /></A>
                 <A href={`/projects/${props.data.slug}`}><MsMedia class="hover:-translate-y-3 def__animate w-full aspect-video object-cover max-h-96 lg:max-h-54 rounded-3xl border-6 border-neutral-200 dark:border-white/5" src={urls[2]} /></A>
            </div>
            <div class="w-full flex justify-center py-18">
                {props.children}
            </div>
            <div ref={msGroup2} class="fade__animate lg:pl-96 flex flex-col lg:flex-row justify-center lg:items-start gap-18 w-full">
                 <A href={`/projects/${props.data.slug}`}><MsMedia class="hover:-translate-y-3 def__animate w-full aspect-video object-cover max-h-96 lg:max-h-54 rounded-3xl border-6 border-neutral-200 dark:border-white/5" src={urls[3]} /></A>
                 <A href={`/projects/${props.data.slug}`}><MsMedia class="hover:-translate-y-3 def__animate w-full aspect-square object-cover max-h-96 lg:max-h-54 rounded-3xl border-6 border-neutral-200 dark:border-white/5 -mt-12" src={urls[4]} /></A>
            </div>
        </div>
    );
}
