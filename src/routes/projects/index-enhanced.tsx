...
export default function ProjectsPage() {
  ...
  const filters = createMemo(() =>
    parseFilterParams(searchParams as Record<string, string>)
  );
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="flex flex-wrap gap-2">
                  <button
                    ...
                    class={`px-3 py-1 rounded-full text-sm transition-colors ${!filters().category || filters().category === 'all'
                  ...
                  <For each={categories()}>
                    {(category) => (
                      <button
                        ...
                        class={`px-3 py-1 rounded-full text-sm transition-colors capitalize ${filters().category === category
                            ? 'bg-black text-white dark:bg-white dark:text-black'
                            : 'bg-black/10 text-black/70 dark:bg-white/10 dark:text-white/70 hover:bg-black/20 dark:hover:bg-white/20'
                          }`}
                      >
                  ...
                  <For each={Array.from({ length: totalPages() }, (_, i) => i + 1)}>
                    {(page) => (
                      <button
                        ...
                        class={`w-8 h-8 rounded-full text-sm transition-colors ${page === filters().page
                            ? 'bg-black text-white dark:bg-white dark:text-black'
